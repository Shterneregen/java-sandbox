...
// equals(String) method this expression returns true. Control goes inside if-block and executes: list.remove('O');.
// remove method is overloaded: remove(int) and remove(Object). char can be easily assigned to int so compiler
// tags remove(int) method. list.remove(<ASCCI value of 'O'>); ASCCI value of 'A' is 65 (this everybody knows)
// so ASCII value of 'O' will be more than 65.
// list.remove('O') throws runtime exception, as it tries to remove an item from the index greater than 65 but
// allowed index is 0 to 3 only.